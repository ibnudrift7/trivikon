...
class ImageHelper {
    ...
    public static function thumb($width, $height, $img, $options = null)
    {
        ...
        if(!file_exists($thumb_path.$thumb_name) || filemtime($thumb_path.$thumb_name) < filemtime($img)){
            ...
            if (function_exists('shell_exec') AND $pathinfo['extension'] == 'png') {
                $compressed_png_content = shell_exec("pngquant --quality=60-$quality --ext -new.png ".escapeshellarg($thumb_path.$thumb_name));
                // if (!$compressed_png_content) {
                //     throw new Exception("Conversion to compressed PNG failed. Is pngquant 1.8+ installed on the server?");
                // }
                if (file_exists($thumb_path."thumb_".$pathinfo['filename'].'_'.$method.'_'.$width.'_'.$height.'-new.png')) {
                    unlink($thumb_path.$thumb_name);
                    rename($thumb_path."thumb_".$pathinfo['filename'].'_'.$method.'_'.$width.'_'.$height.'-new.png', $thumb_path.$thumb_name);
                }
            }
        }
