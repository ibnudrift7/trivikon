 ...
 */
class Literasi extends CActiveRecord
{
	public $title;
	public $content;
	public $writer_name;
	 ...
	 * @return Literasi the static model class
	 */
	...
	public function rules()
	{
		...
		return array(
			array('image, image2, active', 'required'),
			...
			array('data, data[url_facebook], data[url_instagram], data[url_soundcloud], data[url_youtube], data[tanggal], data[jam]', 'safe'),
	...
	public function relations()
	{
		...
		return array(
			'images'=>array(self::HAS_MANY, 'LiterasiImage', 'blog_id'),
			'description'=>array(self::HAS_ONE, 'LiterasiDescription', 'blog_id'),
		);
	}
	...
	public function getData($id, $languageId=1)
	{
		...
		$model = Literasi::model()->find($criteria);
	...
	public function getDataDesc($id, $languageId=1)
	{
		...
		$model = LiterasiDescription::model()->find($criteria);
	...
	public function getAllDataByDate($writer = false, $month, $year, $limit = false, $id = false, $languageId=1)
	{
		...
		$model = Literasi::model()->findAll($criteria);
