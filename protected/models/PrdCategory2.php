 ...
 */
class PrdCategory2 extends CActiveRecord
{
	public $name;
	/**
	 * Returns the static model of the specified AR class.
	 * @param string $className active record class name.
	 * @return PrdCategory2 the static model class
	 */
	...
	public function relations()
	{
		...
		return array(
			'description'=>array(self::HAS_ONE, 'PrdCategory2Description', 'category_id'),
		);
	}
	...
	public function search($language_id)
	{
		...
		$criteria->select = "t.*, prd_category2_description.name";
		$criteria->join = "
		LEFT JOIN prd_category2_description ON prd_category2_description.category_id=t.id
		";
		$criteria->addCondition('prd_category2_description.language_id = :language_id');
		...
		$criteria->compare('prd_category2_description.name',$this->name,true);
	...
	{
		$default = array(
			'select'=>'t.*, prd_category2_description.name',
			'join'=>'LEFT JOIN prd_category2_description ON prd_category2_description.category_id=t.id',
			'order'=>'t.sort ASC',
			/**
			 * @addCondition
			 * criteria @string
	...
	{
		$model = PrdCategory2::model()->getData(array(
			'limit'=>'',
			'addCondition'=>array(
				array(
					'criteria'=>'t.id = :id',
	...
	public function getDataDesc2($id, $languageId=1)
	{
		...
		$model = PrdCategory2Description::model()->find($criteria);
	...
	{
		foreach ($data as $key => $value) {
			$data = PrdCategory2::model()->findByPk($value['id']);
	...
	{
		$data = PrdCategory2::model()->getData(array(
			'limit'=>'',
			'addCondition'=>array(
				array(
					'criteria'=>'type = :type',
	...
	{
		$dataCategory = array();
		$dataAll = PrdCategory2::model()->getDataDesc($categoryId, $languageID, $type);
