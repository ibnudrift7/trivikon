...
class PrdCategory extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('kode, active', 'safe'),
			array('parent_id', 'numerical', 'integerOnly'=>true),
			array('image, image2', 'length', 'max'=>200),
			array('type', 'length', 'max'=>100),

			array('image', 'file', 'types'=>'jpg, gif, png', 'allowEmpty'=>TRUE, 'on'=>'insert', 'except'=>array('createTemp', 'copy')),
	...
	public static function getFilter($idProduct, $languageID)
	{
		$criteria = new CDbCriteria;
		$criteria->addInCondition('t.product_id', $idProduct);
		$criteria->group = 't.name';
		$dataParent = PrdProductFilter::model()->findAll($criteria);
		...
		foreach ($dataParent as $key => $value) {
			...
			$criteria->params[':name'] = $value->name;
			$criteria->addInCondition('t.product_id', $idProduct);
			$criteria->group = 't.value';
			$criteria->order = 't.value ASC';
			$child = PrdProductFilter::model()->findAll($criteria);
			$dataUrutkan = array();
			foreach ($child as $k => $v) {
				$dataUrutkan[] = $v->value;
			}
			if (trim($value->name) == 'Capacity (PK)' OR trim($value->name) == 'Capacity (BTU/Hr)') {
				sort($dataUrutkan, SORT_NUMERIC);
			}
			$data[] = array(
				'data'=>$value,
				'child'=>$dataUrutkan,
