 ...
 */
class Solusi extends CActiveRecord
{
	public $title;
	public $subtitle;
	public $content;
	...
	public function rules()
	{
		...
		return array(
			...
			array('image2, brosur, image3', 'safe'),
			array('brosur', 'file', 'types'=>'docx, pdf, xlsx', 'allowEmpty'=>TRUE, 'on'=>'insert'),
			array('brosur', 'file', 'types'=>'docx, pdf, xlsx', 'allowEmpty'=>TRUE, 'on'=>'update'),
	...
	public function relations()
	{
		...
		return array(
			'images'=>array(self::HAS_MANY, 'SolusiImage', 'solusi_id'),
			'description'=>array(self::HAS_ONE, 'SolusiDescription', 'solusi_id'),
		);
	}
	...
	public function search($language_id)
	{
		...
		$criteria->select = "t.*, pg_solusi_description.title";
		$criteria->join = "
		LEFT JOIN pg_solusi_description ON pg_solusi_description.solusi_id=t.id
		";
		$criteria->addCondition('pg_solusi_description.language_id = :language_id');
	...
	public function getData($id, $languageId=1)
	{
		...
		$criteria->select = "t.*, pg_solusi_description.title, pg_solusi_description.content";
		$criteria->join = "
		LEFT JOIN pg_solusi_description ON pg_solusi_description.solusi_id=t.id
		";
		$criteria->addCondition('pg_solusi_description.language_id = :language_id');
	...
	public function getDataDesc($id, $languageId=1)
	{
		...
		$criteria->addCondition('solusi_id = :id');
		$criteria->params = array(
			':language_id'=>$languageId,
			':id'=>$id,
		);

		$model = SolusiDescription::model()->find($criteria);
	...
	public function getAllData($limit = false, $id = false, $languageId=1)
	{
		...
		// if ($_GET['q'] != '') {
		// 	$criteria->addCondition('(pg_solusi_description.content LIKE :q OR pg_solusi_description.title LIKE :q)');
		// 	$params[':q'] = '%'.$_GET['q'].'%';
		...
		if($_GET['order'] == 'from-a') {
			$criteria->order = "pg_solusi_description.title ASC";
		} elseif($_GET['order'] == 'from-z') {
			$criteria->order = "pg_solusi_description.title DESC";
	...
	public function getMenu($languageId=1, $type = 'archived')
	{
		...
		$criteria->join = "LEFT JOIN pg_solusi_description ON pg_solusi_description.solusi_id=t.id";
		...
		foreach ($data as $key => $value) {
			foreach ($value as $k => $v) {
				$query2 = Yii::app()->db->createCommand("
					SELECT `t`.`id`, `d`.`title`, `d`.`content`, `t`.`date_input` FROM `pg_blog` `t` INNER JOIN `pg_solusi_description` `d`
					ON `t`.`id` = `d`.`solusi_id`
					WHERE
					`t`.`active` = '1' AND
