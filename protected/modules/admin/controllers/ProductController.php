...
class ProductController extends ControllerAdmin
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['PrdProduct']))
		{
			...
			unset($modelSpecific);
			if (count($_POST['PrdTablesSpec']['type']) > 0) {
				foreach ($_POST['PrdTablesSpec']['type'] as $key => $value) {
					$modelSpecific[$key] = new PrdTablesSpec;
					if ($value != '') {
						$modelSpecific[$key]->product_id = $model->id;

						$modelSpecific[$key]->thickness = $_POST['PrdTablesSpec']['thickness'][$key];
						$modelSpecific[$key]->mvtr = $_POST['PrdTablesSpec']['mvtr'][$key];
						$modelSpecific[$key]->o2tr = $_POST['PrdTablesSpec']['o2tr'][$key];
						$modelSpecific[$key]->key_features = $_POST['PrdTablesSpec']['key_features'][$key];

						$modelSpecific[$key]->type = $value;
					}
			...
			if($model->validate() AND $valid){
				...
				try
				{
					...
					PrdTablesSpec::model()->deleteAll('product_id = :id', array(':id'=>$model->id));
					if (count($_POST['PrdTablesSpec']['type']) > 0) {
						foreach ($_POST['PrdTablesSpec']['type'] as $key => $value) {
							$modelSpecific[$key] = new PrdTablesSpec;
							if ($value != '') {
								$modelSpecific[$key]->id_str = $_POST['PrdTablesSpec']['id_str'][$key];
								$modelSpecific[$key]->product_id = $model->id;
								...
								$modelSpecific[$key]->type = $value;
								$modelSpecific[$key]->save(false);
								$modelSpecific[$key]->id_str = $modelSpecific[$key]->id;
								$modelSpecific[$key]->save(false);
							}
	...
	public function actionUpdate($id)
	{
		...
		$modelSpecific = array();
		$modelSpecific = PrdTablesSpec::model()->findAll('product_id = :id ORDER BY id', array(':id'=>$model->id));
