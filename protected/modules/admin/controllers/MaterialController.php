...
class MaterialController extends ControllerAdmin
{
	/**
	...
	public function actionCreate()
	{
		...
		foreach (Language::model()->getLanguage() as $key => $value) {
			$modelDesc[$value->code] = new MaterialDescription;
		}
		...
		if(isset($_POST['Material']))
		{
			...
			foreach ($_POST['MaterialDescription'] as $j => $mod) {
	            if (isset($_POST['MaterialDescription'][$j])) {
	                $modelDesc[$j]=new MaterialDescription; // if you had static model only
			...
			$model->data = $_POST['Material']['data'];

			if($model->validate() AND $valid){
				$transaction=$model->dbConnection->beginTransaction();
				try
				{
					$image->saveAs(Yii::getPathOfAlias('webroot').'/images/material/'.$model->image);
					// $image2->saveAs(Yii::getPathOfAlias('webroot').'/images/material/'.$model->image2);
					// $model->date_input = date("Y-m-d H:i:s");
					...
					foreach ($modelDesc as $key => $value) {
						$value->material_id=$model->id;
						$value->save();
					}

					Log::createLog("Material Controller Create $model->id");
	...
	public function actionUpdate($id)
	{
		...
		$modelDesc = array();
		foreach (Language::model()->getLanguage() as $key => $value) {
			$modelDesc[$value->code] = Material::model()->getDataDesc($model->id, $value->id);
			$modelDesc[$value->code] = ($modelDesc[$value->code]==null) ? new MaterialDescription : $modelDesc[$value->code];
		...
		if(isset($_POST['Material']))
		{
			...
			$model->attributes=$_POST['Material'];//setting semua nilai
			...
			if($model->validate() AND $valid){
				...
				try
				{
					...
					MaterialDescription::model()->deleteAll('material_id = :id', array(':id'=>$model->id));
					foreach ($modelDesc as $key => $value) {
						$value->material_id=$model->id;
						$value->save();
					}

					Log::createLog("MaterialController Update $model->id");
