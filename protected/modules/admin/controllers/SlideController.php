...
class SlideController extends ControllerAdmin
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Slide']))
		{
			...
			$model->image = 'slide/'.substr(md5(time()),0,5).'-'.$image->name;

			$image2 = CUploadedFile::getInstance($model,'image2');
			if ($image2->name != '') {
				$model->image2 = 'slide/'.substr(md5(time()),0,5).'-'.$image2->name;
			}
			...
			if($model->validate() AND $valid){
				...
				try
				{
					...
					if ($image2->name != '') {
						$image2->saveAs(Yii::getPathOfAlias('webroot').'/images/'.$model->image2);
					}
